...
INNER JOIN 
    jobs_applied ON job_id.id = jobs_applied.job_id
WHERE 
    jobs_applied.candidate_id = $candidate_id
";
...
if (!empty($filters['location']) && is_array($filters['location'])) {
  ...
  $city_query = "SELECT city FROM locations WHERE id IN ($id_list)";
  $city_result = mysqli_query($conn, $city_query);

  $cities = [];
  while ($row = mysqli_fetch_assoc($city_result)) {
    $cities[] = $conn->real_escape_string($row['city']);
  }
  $matching_ids = [];
  foreach ($cities as $city) {
    $like_query = "SELECT id FROM locations WHERE city LIKE '%$city%'";
