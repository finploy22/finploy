...
function timeAgo($timestamp)
{
  ...
  if ($diff < 60) {
    ...
    return round($diff / 2629440) . " months ago";
  } else {
...
$jobidurl = isset($_GET['jobidurl']) ? intval($_GET['jobidurl']) : 0;
if ($jobidurl > 0) {
  $query .= " AND job_id.id = $jobidurl";
} else {
  ...
  if (!empty($filters['location']) && is_array($filters['location'])) {
    ...
    $escaped_ids = array_map('intval', $filters['location']);
    $id_list = implode(',', $escaped_ids);

    // Step 2: Get city_wise_ids for the selected location ids
    $city_query = "SELECT DISTINCT city_wise_id FROM locations WHERE id IN ($id_list)";
    $city_result = mysqli_query($conn, $city_query);

    $city_wise_ids = [];
    while ($row = mysqli_fetch_assoc($city_result)) {
      $city_wise_ids[] = $conn->real_escape_string($row['city_wise_id']);
    }

    // Step 3: Use city_wise_ids to get all matching location IDs
    if (!empty($city_wise_ids)) {
      $city_id_list = implode("','", $city_wise_ids);  // String list for SQL IN clause
      $location_query = "SELECT id FROM locations WHERE city_wise_id IN ('$city_id_list')";
      $location_result = mysqli_query($conn, $location_query);

      $all_location_ids = [];
      while ($row = mysqli_fetch_assoc($location_result)) {
        $all_location_ids[] = (int) $row['id'];
      }

      // Step 4: Apply to main query
      if (!empty($all_location_ids)) {
        $all_ids_list = implode(',', $all_location_ids);
...
// exit;
  ...
  // For companyname new for seo 
  if (!empty($filters['companyname']) && is_array($filters['companyname'])) {
    $safe = array_map([$conn, 'real_escape_string'], $filters['companyname']);
    $safe_list = "'" . implode("','", $safe) . "'";
    $query .= " AND companyname IN ($safe_list)";
  }
...
if ($result && $result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    ...
    $candidate_query = "SELECT user_id FROM candidates WHERE mobile_number = '$mobile'";
    ...
    // need style 
    // style="padding-left: 6px; word-break: break-word; max-width: 200px;" 
    ...
    $button = (mysqli_num_rows($applied_result) > 0)
      ...
      : '<button class="btn btn-success" id="job-apply-btn"><img src=images/codicon_git-stash-apply.svg  width="16" height="16" style="margin: 0 4px 4px 0;"> Apply for Job</button>';
